...
> 说明：本文件在早期用于规划 “V0.2 → V0.5” 的 Phase 1-4。  
> 现在 Phase 1/2 已完成、Phase 3 完成核心部分。本文顶部给出最新路线图；原始清单保留在“附录（历史）”供追溯。

---

## 🎯 后续开发路线（建议）

### Milestone A：V0.6 RC（把 Phase 3 补齐到可发布）
**目标**：把“剩余 20%（Phase 3）”做成用户可用的完整闭环，进入对外测试。  
**建议周期**：1-2 周

**P0（必须交付）**
1. **Agent 选择器 UI**：Webview 中提供选择/切换入口，状态可见（在线/离线/错误）。
2. **权限规则管理 UI**：能查看/删除会话内保存的规则；支持按 tool/category 过滤。
3. **Webview 通信与渲染性能**：消息批量发送（减少 `postMessage` 频率）+ 长会话滚动稳定。
4. **大文件安全与体验**：大文件（例如 >1MB）读取/写入提示 + 分片读取策略；diff 展示避免卡死（必要时降级为“仅展示摘要/跳转到文件”）。

**P1（尽量交付）**
1. **内置 MCP 工具补全**：优先补 `git/diff`、`git/log`、`lsp/getDefinition`、`lsp/getReferences`。
2. **批量权限审批**：当出现连续/并发的请求时，提供“对同类请求一键允许/拒绝”的二次确认交互。

**验收标准（V0.6）**
- 新用户不看文档可完成：创建会话 → Plan/Execute → 审批 → 终端输出 → diff 审阅 → 写入落盘。
- 连续输出（终端/流式回复）下 UI 不丢帧、不乱序，长会话滚动不跳。
- 大文件场景不会把 UI 卡死（有明确降级路径）。

---

### Milestone B：V1.0（正式发布质量）
**目标**：面向 VSCode Marketplace 的稳定性、可观测性与文档齐备。  
**建议周期**：4-8 周（与 Beta 反馈并行迭代）

**P0（必须交付）**
1. **发布与安装体验**：打包产物、平台兼容（尤其 `node-pty`）、版本升级策略。
2. **测试与回归**：关键路径的单测/集成测试 + 最小 E2E（启动扩展、发起会话、模拟审批）。
3. **文档体系**：安装/配置/权限模型/故障排查/隐私与安全说明。
4. **崩溃与诊断**：日志导出与关键指标（会话恢复、Agent 重启、MCP 失败）可定位。

**P1（可选交付）**
1. Phase 4 选择性实现：工作流自动化/上下文管理/协作分享（以明确的“可裁剪范围”推进）。

**验收标准（V1.0）**
- “默认安全”：Workspace Trust + 权限审批 + 审计导出都能稳定工作。
- “可诊断”：用户遇到失败能导出一份足够排查的日志/审计包。
- “可维护”：路线图/PRD/技术方案/进度文档口径一致，能驱动迭代。

---

## ✅ 现状与差距（摘要）
- Phase 1（UX）：已完成（见 `PHASE1-COMPLETION-REPORT.md`）
- Phase 2（Reliability）：已完成（见 `PHASE2-COMPLETION-REPORT.md`）
- Phase 3（Features）：核心已完成，剩余项集中在 UI/性能/大文件/LSP（见 `PHASE3-COMPLETION-REPORT.md` 的“未完成项”）
- 进度管理与发布流程：见 `docs/PROJECT-TRACKING.md`

---

## 附录（历史）：V0.2 时点的原始 Phase 规划（保留）

> 注意：以下内容为 V0.2 时点的原始规划/清单，包含已完成或已被替换的方案细节（例如实现选型与任务勾选状态），请以本文顶部的 Milestone A/B 与各 Phase 完成报告为准。
