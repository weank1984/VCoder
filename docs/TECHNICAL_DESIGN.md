# VCoder - 技术方案设计文档

> 基于 CLI Agent 的 VSCode 智能编程助手插件（支持 Claude Code、Codex CLI）

**版本**: v0.1.0  
**日期**: 2026-01-01  
**状态**: 草稿  

---

## 1. 技术架构概述

### 1.1 整体架构

VCoder 采用 **Client-Server 分离架构**，核心理念是将 CLI Agent（如 Claude Code、Codex CLI）作为后台运行，前端 VSCode 插件通过统一的 ACP 协议进行通信。

```
┌─────────────────────────────────────────────────────────────────────────┐
│                          VSCode Extension (Client)                      │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │                        Extension Host                            │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌────────────────────┐    │   │
│  │  │ ChatProvider │  │ DiffProvider │  │ FileDecorator      │    │   │
│  │  │ (Webview)    │  │              │  │                    │    │   │
│  │  └──────┬───────┘  └──────┬───────┘  └─────────┬──────────┘    │   │
│  │         └─────────────────┼─────────────────────┘              │   │
│  │                           ▼                                     │   │
│  │                  ┌────────────────┐                            │   │
│  │                  │   ACPClient    │                            │   │
│  │                  │  (JSON-RPC)    │                            │   │
│  │                  └───────┬────────┘                            │   │
│  └──────────────────────────┼──────────────────────────────────────┘   │
└──────────────────────────────┼──────────────────────────────────────────┘
                               │ stdio (stdin/stdout)
┌──────────────────────────────┼──────────────────────────────────────────┐
│                              ▼                         Agent Server     │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │                      CLI Adapter (抽象层)                        │  │
│  │  • 统一接口：prompt / acceptChange / rejectChange                │  │
│  │  • 输出解析：将不同 CLI 输出转换为统一的 ACP 消息               │  │
│  └───────────────────────┬──────────────────┬───────────────────────┘  │
│                         │                  │                           │
│                         ▼                  ▼                           │
│  ┌─────────────────────────┐  ┌───────────────────────────────────┐  │
│  │ ClaudeCodeWrapper     │  │ CodexCliWrapper (未来)        │  │
│  │ (MVP)                 │  │                               │  │
│  └───────────┬─────────────┘  └───────────────┬───────────────────┘  │
│              │                              │                           │
│              ▼                              ▼                           │
│  ┌─────────────────────────┐  ┌───────────────────────────────────┐  │
│  │ Claude Code CLI       │  │ OpenAI Codex CLI              │  │
│  │ (@anthropic-ai)       │  │ (@openai/codex)               │  │
│  └─────────────────────────┘  └───────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────┘
```

### 1.2 技术选型

| 层级 | 技术选择 | 理由 |
|------|---------|------|
| Extension Host | TypeScript | VSCode Extension 原生语言 |
| UI (Webview) | React + TypeScript | 组件化开发，生态丰富 |
| 状态管理 | Zustand | 轻量级，无样板代码 |
| 通信协议 | ACP over JSON-RPC 2.0 | 标准化 Agent 通信协议 |
| Agent Server | TypeScript (Node.js) | 与 Extension 共享类型定义 |

### 1.3 职责划分

| 功能 | 我们实现 | CLI Agent 实现 |
|------|:-------:|:---------------:|
| 模型调用 | ❌ | ✅ |
| 工具定义和执行 | ❌ | ✅ |
| MCP 调用 | ❌ | ✅ |
| 思考过程生成 | ❌ | ✅ |
| Token 上下文管理 | ❌ | ✅ |
| ACP 协议转换 | ✅ | ❌ |
| UI 渲染 | ✅ | ❌ |
| Diff 预览 | ✅ | ❌ |
| 用户确认流程 | ✅ | ❌ |

### 1.4 多后端支持规划

> **设计原则**：通过抽象层支持多个 CLI Agent 后端，保持上层 ACP 协议不变。

#### 1.4.1 后端对比

| 特性 | Claude Code CLI | Codex CLI |
|------|----------------|-----------|
| **厂商** | Anthropic | OpenAI |
| **安装** | `npm install -g @anthropic-ai/claude-code` | `npm install -g @openai/codex` |
| **非交互模式** | `-p` 标志 | `codex exec` 命令 |
| **JSON 输出** | `--output-format stream-json` | `--json` (JSONL) |
| **MCP 支持** | ✅ | ✅ |
| **TypeScript SDK** | 需自行包装 | ✅ `@openai/codex-sdk` |
| **状态** | ✅ MVP | 📅 后续 |

#### 1.4.2 事件类型映射

| ACP 统一类型 | Claude Code 事件 | Codex CLI 事件 |
|-------------|-----------------|----------------|
| `thought` | `thinking` | `reasoning` |
| `text` | `text` | `agent_message` |
| `tool_use` | `tool_use` | `command_execution` (started) |
| `tool_result` | `tool_result` | `command_execution` (completed) |
| `file_change` | `file_write` | `file_change` |
| `mcp_call` | `mcp_tool_use` | `mcp_tool_call` |
| `task_list` | `TodoWrite` | `todo_list` |
| `subagent_run` | `Task` | - |
| `complete` | 进程退出 | `turn.completed` |

#### 1.4.3 兼容性设计

1. **统一接口 (CliAdapter)**
   - `prompt(sessionId, message)` → 启动 CLI 并发送消息
   - `acceptChange(path)` → 接受文件修改
   - `rejectChange(path)` → 拒绝文件修改
   - `onUpdate(callback)` → 接收流式更新

2. **输出解析层**
   - 每个 CLI 的 Wrapper 负责将其特定输出转换为统一的 ACP 消息
   - 新增 CLI 只需实现对应的 Wrapper

3. **配置切换**
   - 用户可在设置中选择默认 CLI Agent
   - 未来支持在当前会话中切换

---

## 2. 模块设计

### 2.1 项目结构

```
vcoder/
├── packages/
│   ├── extension/           # VSCode 插件
│   │   ├── src/
│   │   │   ├── extension.ts        # 入口
│   │   │   ├── acp/client.ts       # ACP 客户端
│   │   │   ├── providers/          # Webview、Diff、装饰器
│   │   │   └── services/           # 服务管理器
│   │   └── webview/                # React Webview UI
│   │       └── components/         # UI 组件
│   │
│   ├── server/              # Agent Server (Claude Code 包装层)
│   │   └── src/
│   │       ├── acp/                # ACP 服务端
│   │       └── claude/wrapper.ts   # Claude Code CLI 包装
│   │
│   └── shared/              # 共享类型定义
│
├── package.json             # Monorepo 根配置
└── turbo.json               # Turborepo 配置
```

### 2.2 Webview 组件

| 组件 | 职责 |
|------|------|
| `ChatBubble` | 用户/AI 对话气泡 |
| `ThoughtBlock` | 可折叠的思考过程块 |
| `ToolCallList` | 工具调用列表（"Finished working"） |
| `TaskList` | 任务计划列表（Plan Mode） |
| `TaskRuns` | 子代理/并行执行列表（`Task` 工具） |
| `ModelSelector` | 模型选择下拉框 |
| `PlanModeToggle` | Plan Mode 切换按钮 |
| `InputArea` | 输入区（含附件、@引用） |
| `SessionHeader` | 会话管理头部 |
| `SessionList` | 多会话列表面板 |
| `ConfirmDialog` | 确认弹窗（Plan/Bash执行） |
| `ErrorBanner` | 错误提示条 |
| `ApiKeySetup` | API Key 配置向导 |

---

## 3. ACP 协议设计

### 3.1 核心消息类型

| 方法 | 方向 | 说明 |
|------|------|------|
| `initialize` | Client → Server | 初始化连接，声明能力 |
| `session/new` | Client → Server | 创建新会话 |
| `session/list` | Client → Server | 获取会话列表 |
| `session/switch` | Client → Server | 切换当前会话 |
| `session/delete` | Client → Server | 删除会话 |
| `session/prompt` | Client → Server | 发送用户消息 |
| `session/update` | Server → Client | 流式更新 |
| `session/complete` | Server → Client | 会话回合完成 |
| `settings/change` | Client → Server | 切换模型/模式 |
| `file/accept` | Client → Server | 接受文件修改 |
| `file/reject` | Client → Server | 拒绝文件修改 |
| `bash/confirm` | Client → Server | 确认执行 bash 命令 |
| `bash/skip` | Client → Server | 跳过 bash 命令 |
| `plan/confirm` | Client → Server | 确认执行计划 |

### 3.2 更新类型 (session/update)

| type | 说明 | 关键字段 |
|------|------|---------|
| `thought` | 思考过程 | content, isComplete |
| `text` | 文本输出 | text |
| `tool_use` | 工具调用开始 | id, name, input, status |
| `tool_result` | 工具调用结果 | id, result, error |
| `file_change` | 文件修改 | path, diff, proposed |
| `mcp_call` | MCP 工具调用 | server, tool, status |
| `task_list` | 任务计划列表 | tasks[], currentTaskId |
| `subagent_run` | 子代理/并行执行（`Task` 工具） | id, title, subagentType, status, parentTaskId? |
| `bash_request` | Bash 命令确认请求 | command, id |
| `plan_ready` | 计划完成待确认 | tasks[], summary |
| `error` | 错误信息 | code, message, action |

> **语义说明**：
> - `task_list`：由 Claude Code `TodoWrite` 工具输入（通常为 `todos[]`）转换而来，是“用户可见计划/进度”的主数据源。
> - `subagent_run`：由 Claude Code `Task` 工具调用转换而来，代表“子代理/并行执行单元”，用于在 UI 中单独展示并行执行进度；不等同于 `task_list`。

### 3.3 通信流程

```
Client                              Server                          Claude Code
  │                                    │                                  │
  │─── initialize ───────────────────►│                                  │
  │◄── initialized ──────────────────-│                                  │
  │                                    │                                  │
  │─── session/new ──────────────────►│                                  │
  │◄── session/created ──────────────-│                                  │
  │                                    │                                  │
  │─── session/prompt ───────────────►│─── spawn CLI ───────────────────►│
  │                                    │◄── JSON stream ──────────────────│
  │◄── session/update (thought) ──────│                                  │
  │◄── session/update (tool_use) ─────│                                  │
  │◄── session/update (tool_result) ──│                                  │
  │◄── session/update (file_change) ──│                                  │
  │◄── session/update (text) ─────────│                                  │
  │◄── session/complete ──────────────│                                  │
  │                                    │                                  │
  │─── file/accept ──────────────────►│─── confirm change ──────────────►│
  │◄── file/applied ─────────────────-│                                  │
```

---

## 4. Agent Server 设计

### 4.1 核心职责

Agent Server 是 Claude Code CLI 的**协议适配层**：

1. **进程管理**：启动/关闭 Claude Code CLI 子进程
2. **协议转换**：将 Claude Code JSON 输出转换为 ACP 消息
3. **会话管理**：维护多会话状态

### 4.2 Claude Code CLI 包装器 (伪代码)

```
class ClaudeCodeWrapper:
  
  function prompt(sessionId, message):
    # 启动 Claude Code CLI 子进程
    process = spawn("claude", [
      "-p", message,
      "--output-format", "stream-json",
      "--model", currentModel
    ])
    
    # 逐行读取 JSON 输出
    for each line in process.stdout:
      event = parseJSON(line)
      acpMessage = convertToACP(event)
      emit("update", sessionId, acpMessage)
    
    # 完成
    emit("complete", sessionId)

  function convertToACP(claudeEvent):
    match claudeEvent.type:
      "thinking"    → { type: "thought", content: ... }
      "text"        → { type: "text", text: ... }
      "tool_use"    → { type: "tool_use", id: ..., name: ..., status: ... }
      "tool_result" → { type: "tool_result", id: ..., result: ... }
      "file_write"  → { type: "file_change", path: ..., diff: ... }
      "mcp_tool"    → { type: "mcp_call", server: ..., tool: ... }

    # Special tool projections (semantic updates)
    if tool.name == "TodoWrite":
      emit { type: "task_list", tasks: mapTodos(tool.input.todos) }
    if tool.name == "Task":
      emit { type: "subagent_run", id: tool.id, title: tool.input.description, subagentType: tool.input.subagent_type, status: "running" }
```

---

## 5. Extension Host 设计

### 5.1 启动流程 (伪代码)

```
function activate(context):
  # 1. 启动 Agent Server
  serverManager = new ServerManager()
  serverManager.start()

  # 2. 初始化 ACP 客户端
  acpClient = new ACPClient(serverManager.stdio)
  acpClient.initialize()

  # 3. 注册 Webview Provider
  chatProvider = new ChatViewProvider(acpClient)
  registerWebviewViewProvider("vcoder.chatView", chatProvider)

  # 4. 注册文件装饰器
  fileDecorator = new FileDecorator(acpClient)
  registerFileDecorationProvider(fileDecorator)

  # 5. 注册状态栏
  statusBar = createStatusBarItem()
  statusBar.text = "VCoder: ● Connected"
```

### 5.2 Webview 通信 (伪代码)

```
class ChatViewProvider:
  
  function resolveWebviewView(webview):
    # 设置 Webview HTML
    webview.html = loadReactApp()
    
    # 监听来自 Webview 的消息
    webview.onMessage(message):
      match message.type:
        "send"         → acpClient.prompt(sessionId, message.content)
        "acceptChange" → acpClient.acceptFileChange(message.path)
        "rejectChange" → acpClient.rejectFileChange(message.path)
        "setModel"     → acpClient.changeSettings({ model: message.model })
        "setPlanMode"  → acpClient.changeSettings({ planMode: message.enabled })
    
    # 将 ACP 更新转发给 Webview
    acpClient.on("session/update", data):
      webview.postMessage({ type: "update", data })
```

---

## 6. 构建与部署

### 6.1 开发命令

| 命令 | 说明 |
|------|------|
| `pnpm install` | 安装依赖 |
| `pnpm build` | 并行构建所有包 |
| `pnpm dev` | 开发模式（监听变更） |
| `vsce package` | 打包 VSIX |
| `vsce publish` | 发布到 Marketplace |

### 6.2 依赖条件

| 依赖项 | 版本要求 |
|--------|---------|
| Node.js | ≥ 18.0.0 |
| VSCode | ≥ 1.80.0 |
| Claude Code CLI | 最新版 |

---

## 7. 安全考量

| 安全点 | 措施 |
|--------|------|
| API Key 存储 | 使用 VSCode Secret Storage API |
| 命令执行 | 文件写入需用户确认 (Diff 预览) |
| 工具访问 | 限制在 workspace 范围内 |

---

## 8. 测试策略

| 测试类型 | 覆盖范围 |
|---------|---------|
| 单元测试 | ACP 消息序列化、协议转换 |
| 集成测试 | Extension 激活、服务启动 |
| 手动测试 | 对话流程、Diff 预览、文件操作 |

---

## 9. 风险与缓解

| 风险 | 影响 | 缓解措施 |
|------|------|---------|
| Claude Code CLI 更新不兼容 | 高 | 封装隔离层，锁定版本 |
| ACP 协议尚未稳定 | 中 | 设计抽象层，便于升级 |
| 长时间运行内存泄漏 | 中 | 监控内存，定期重启 |

---

## 附录

### 参考资料

- [ACP 协议规范](https://agentclientprotocol.com)
- [VSCode Extension Guide](https://code.visualstudio.com/api)
- [Claude Code 官方文档](https://docs.anthropic.com/claude-code)

### 术语表

| 术语 | 说明 |
|------|------|
| ACP | Agent Client Protocol，AI Agent 与 IDE 通信协议 |
| JSON-RPC | 轻量级远程过程调用协议 |
| MCP | Model Context Protocol，模型上下文协议 |
